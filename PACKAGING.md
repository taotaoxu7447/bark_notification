...
The shared CLI contract is `agentwatch install --delivery bark|agentwatch|both`; each platform installer forwards `--delivery`. There is no `configure-bark` command. Bark URL/key and AgentWatch password must never enter AI chat or argv. Install and `update` idempotently merge AgentWatch's official Claude Code `Stop`/`StopFailure` hooks into the active user settings, preserve every unrelated setting and hook, and never add `SubagentStop`. Claude Code 2.1.196 or newer is required: exec-form `args` appeared earlier, while this notifier also relies on the modern Stop background-task fields and stable `prompt_id`. Install, `update`, and `doctor` never send a test notification, and `doctor` never starts/restarts the background watcher.
...
3. Confirm install/update/login do not automatically send any test notification. Run `--test-claude` only as a user-requested, explicit one-shot test.
...
6. Confirm install/update add exactly the managed Claude `Stop` and `StopFailure` hooks, preserve existing `~/.claude/settings.json` hooks/settings, remain idempotent, and never enable `SubagentStop`.
7. Confirm the Claude hook only appends to the private local spool; network delivery occurs in the background watcher. Confirm uninstall removes only AgentWatch's own Claude hook entries.
8. Confirm first-run baseline does not replay old Codex, ZCode, Kimi Code, Grok Build, or Claude hook-spool history.
9. Confirm an event is attempted at most twice and a channel that succeeded in round one is never sent again in round two.
10. Confirm Android exposes a dedicated Claude icon, notification channel, and history source, and every desktop package contains `assets/claude-icon-v1.png`.
11. Build all three package files from the same git commit.
12. Tag the commit `v0.3.0`.
13. Run `android/build_release.zsh`, verify the APK signature/application ID, and
    publish `AgentWatch-android-v0.3.0.apk` from the same commit.
