...
| --- | --- | --- |
| `bark` | Install Bark on iPhone; Apple Watch mirrors the iPhone notification. Do not install, register, or log in to AgentWatch. | From Bark's bottom **Server** tab, use the first card and store only its base personal push URL ending at the device key. This is not AgentWatch account pairing. |
...
The first Bark Server card shows a test URL shaped like `https://api.day.app/<device-key>/replace-with-your-push-content`. The configured `BARK_URL` must remove the sample-content suffix and end at `<device-key>`. Do not choose the later title, sound, continuous-ring, or archive example URLs. `BARK_KEY` is shorthand only for the official `api.day.app`; self-hosted Bark requires the full base URL. The URL contains its key and is therefore a secret. The user must personally place it in private computer configuration; it must not enter AI chat, argv, logs, or Git. The same no-chat/no-argv rule applies to the AgentWatch password, which is entered only through the CLI's hidden interactive prompt.
...
- Codex and ZCode Bark groups/icons are shipped as defaults in `env.example`; ordinary users do not set them individually. Optional overrides are `CODEX_BARK_ICON` / `CODEX_BARK_GROUP` and `ZCODE_BARK_ICON` / `ZCODE_BARK_GROUP`. The default assets are `assets/codex-icon-large-v1.png` and `assets/zcode-icon-v1.png`. ZCode notifications watch `~/.zcode/cli/log/zcode-*.jsonl`.
- Kimi Code, Grok Build, Claude Code, Pi Agent, and OpenCode groups/icons are also built-in defaults. Their `*_BARK_ICON` and `*_BARK_GROUP` variables are optional overrides, not installation requirements.
...
- Bark settings: the personal base `BARK_URL` comes from the first card on Bark's iPhone **Server** tab, with the sample push-content path removed so the URL ends at the device key. `BARK_KEY` is valid as a shorthand only for the official `api.day.app`. It must be stored privately on the computer. It is a Bark delivery credential, not AgentWatch pairing or an AgentWatch subscription URL. Codex, ZCode, Kimi Code, Grok Build, Claude Code, Pi Agent, and OpenCode groups/icons are built-in defaults rather than required user input.
...
- AgentWatch v0.4.3 uses the SH VPS endpoint `https://aw.taotaoxu.net`. A stored Android private session from `https://191.222.219.94:9444` is migrated locally without changing its username, topic, or tokens; desktop `update` rewrites only the retired project default API base and preserves custom endpoints. Registration responses still require the current domain.

## What It Monitors

- Cursor uses the official user-level `stop` hook: `conversation_id`, `generation_id`, `status`, `loop_count`, and the first workspace root. It writes one bounded private queue record, with no email, model reasoning, or transcript content. Completed, error, and aborted outcomes share the existing delivery pipeline. Config merging preserves unrelated events/handlers, keeps an initial backup, and removes only a marked AgentWatch command. Imported Claude hooks are ignored when Cursor identifies the process/payload, preventing duplicate Claude notifications. Other hooks may request a follow-up after stop; this reports a turn boundary, not permanent idle. Cloud agents do not support this user hook. Reference: https://cursor.com/docs/hooks . Native IDE/CLI end-to-end triggering still needs validation on a installed version.
